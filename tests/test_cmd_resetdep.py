...
class TestCmdResetDep(object):
    ...
    def test_file_dep(self, dep_manager, dependency1):
        my_task = Task("t2", [""], dependencies=[FileDependency('tests/data/dependency1')])
        output = StringIO()
        ...
        # State is stored under absolute path (get_key())
        dep = my_task.dependencies[0].get_key()
    ...
    def test_missing_file_dep(self, dep_manager):
        my_task = Task("t2", [""], dependencies=[FileDependency('tests/data/missing')])
        output = StringIO()
        cmd_reset = CmdFactory(ResetDep, outstream=output, task_list=[my_task],
                               dep_manager=dep_manager)
        cmd_reset._execute()
    ...
    def test_missing_dep_and_target(self, dep_manager, dependency1, dependency2):
        ...
        task_a = Task("task_a", [""],
                      dependencies=[FileDependency('tests/data/dependency1')],
                      targets=['tests/data/dependency2'])
        task_b = Task("task_b", [""],
                      dependencies=[FileDependency('tests/data/dependency2')],
                      targets=['tests/data/dependency3'])
        task_c = Task("task_c", [""],
                      dependencies=[FileDependency('tests/data/dependency3')],
    ...
    def test_values_and_results(self, dep_manager, dependency1):
        ...
        reseted = Task("t2", [""], dependencies=[FileDependency('tests/data/dependency1')])
        output = StringIO()
